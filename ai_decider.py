...
3. Технический сигнал (print_signal_confirmed=false, log_download_error=true,
   ...
   У тебя ДВА независимых источника, и оба заполняются всегда:
   - print_signal_confirmed — SNMP-история принтера. Пишется на сервере постоянно,
     не зависит от того, на связи ли ПК аппарата. Но она знает только, что принтер
     переходил в состояние "печатает" — не какой именно файл.
   - log_download_error/log_print_success — логи ПК аппарата. Они привязаны к имени
     ИМЕННО ЭТОГО документа, поэтому точнее отвечают на вопрос "прошёл ли файл юзера".

   Как их сводить:
   - оба говорят "напечаталось" — печать точно была, см. пункт 4;
   - оба говорят "нет" — явный сбой, рекомендуй возврат с confidence="high";
   - SNMP="напечаталось", а в логах ошибка по файлу юзера — доверяй ЛОГАМ: SNMP мог
     зафиксировать соседний заказ на том же аппарате. Это повод рекомендовать возврат,
     confidence="medium", и честно написать сотруднику про расхождение;
   - SNMP молчит (null), а логи говорят "успешно" — печать всё-таки прошла, SNMP просто
     не попал в опрос. Отвечай как в пункте 4;
   - всё null — технических данных за этот период реально нет (аппарат и ПК были не в
     сети). Не угадывай по интуиции, склоняйся к escalate с честной пометкой
     "технических данных за это время нет".
...
_HANDOFF_VERB = (
    ...
    r"вызов(?:у|ем|ём)|позов(?:у|ем|ём)|пришл(?:ю|ем|ём)|отправ(?:лю|им)|"
...
_HANDOFF_WHO = r"сотрудник|специалист|механик|мастер|команд|кого-нибудь|кого-то|қызметкер|маман"
...
человека. Ответь сам и закрой: делать этого не нужно, аппараты обслуживаем мы.
